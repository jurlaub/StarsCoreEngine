...
class Hull(BaseTech):
    ...
    def __init__(self):
        ...
        self.spaceDockSize = None   # see static method - hull_space_dock_size()
    ...
    @staticmethod
    def hull_space_dock_size():
        """  ------------ spaceDockSize ---------------
            Hulls have a space dock size that is integral to the hull 
            values come from a game's technology template but there are a few 
            hardcoded values

        Values:
            > None = no space dock size and none possible
            > 0 = no currently supported size 
            > n = a signed int representing the mass limit for ship construction
                    at a space dock 
            > -1 = INFINITY - any mass ship can be constructed 


        """
        max_size = {"max_size_supported" : Hull.INFINITY }
...
#                 if kee == 'spaceDockSize':
#                     #sizeRank = ('0', '200', '-1')
#                     sizeRank = (0, 200, ShipDesign INFINITY) 
...
#                         print("spaceDockSize: existing:%s and potential %s " % (sds, obj) )
#                         if sds in sizeRank:
#                             #compare index
...
#                         tmpVal = obj
#                     else:
#                         print("value not in %s  - value is %s" % (sizeRank, obj))
...
#             else:
#                 if DEBUG: print("tally ignoring value: %s : %s" % (kee, obj )) 
